...
def torchscript_to_coreml(labels, sample_input, torchscript_model_path='models/model.pt', coreml_model_path='models/Model.mlpackage'):
...
@click.option("--softmax", type=bool, default=False)
def main(mode: str, softmax: bool):
    ...
    labels = list(pd.read_csv('assets/labels.csv', header=None)[0])
    ...
    # verification test
    image = Image.open('assets/image.jpeg')
    ...
    if coreml_model:
        coreml_out = coreml_model.predict({'image': image.resize((height, width))})['classLabel_probs']
    ...
    for _ in tqdm(range(100)):
        ...
        if coreml_model:
            coreml_out = coreml_model.predict({'image': random_image})['classLabel_probs']
        ...
        # measure conversion quality here:

        abs_errors['torchscript'].append(
            np.abs(torch_out-torchscript_out).mean()
        )
        rel_errors['torchscript'].append(
            (np.abs(torch_out-torchscript_out) / np.abs(torch_out)).mean()
        )

        abs_errors['onnx'].append(
            np.abs(torch_out-onnx_out).mean()
        )
        rel_errors['onnx'].append(
            (np.abs(torch_out-onnx_out) / np.abs(torch_out)).mean()
        )

        abs_errors['tflite'].append(
            np.abs(torch_out-tflite_out).mean()
        )
        rel_errors['tflite'].append(
            (np.abs(torch_out-tflite_out) / np.abs(torch_out)).mean()
        )

        if coreml_model:
            abs_errors['coreml'].append(
                np.abs(torch_out-coreml_out).mean()
            )
            rel_errors['coreml'].append(
                (np.abs(torch_out-coreml_out) / np.abs(torch_out)).mean()
            )
